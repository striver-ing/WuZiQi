...
int lua_wuziqi_NetworkManagerProtocol_clearReceivedMessageCallback(lua_State* tolua_S)
{
    int argc = 0;
...
#if COCOS2D_DEBUG >= 1
    if (!tolua_isusertype(tolua_S,1,"NetworkManagerProtocol",0,&tolua_err)) goto tolua_lerror;
#endif
    ...
    if (!cobj) 
    {
        tolua_error(tolua_S,"invalid 'cobj' in function 'lua_wuziqi_NetworkManagerProtocol_clearReceivedMessageCallback'", nullptr);
        return 0;
    }
#endif
    ...
    if (argc == 0) 
    {
        if(!ok)
        {
            tolua_error(tolua_S,"invalid arguments in function 'lua_wuziqi_NetworkManagerProtocol_clearReceivedMessageCallback'", nullptr);
            return 0;
        }
        cobj->clearReceivedMessageCallback();
        lua_settop(tolua_S, 1);
        return 1;
    }
    luaL_error(tolua_S, "%s has wrong number of arguments: %d, was expecting %d \n", "NetworkManagerProtocol:clearReceivedMessageCallback",argc, 0);
    return 0;

#if COCOS2D_DEBUG >= 1
    tolua_lerror:
    tolua_error(tolua_S,"#ferror in function 'lua_wuziqi_NetworkManagerProtocol_clearReceivedMessageCallback'.",&tolua_err);
#endif
...
int lua_wuziqi_NetworkManagerProtocol_sendMessage(lua_State* tolua_S)
{
    ...
    if (argc == 1) 
    {
        ...
        std::string arg0_tmp; ok &= luaval_to_std_string(tolua_S, 2, &arg0_tmp, "NetworkManagerProtocol:sendMessage"); arg0 = arg0_tmp.c_str();
        if(!ok)
        {
...
int lua_register_wuziqi_NetworkManagerProtocol(lua_State* tolua_S)
{
    ...
    tolua_beginmodule(tolua_S,"NetworkManagerProtocol");
        tolua_function(tolua_S,"clearReceivedMessageCallback",lua_wuziqi_NetworkManagerProtocol_clearReceivedMessageCallback);
...
#if COCOS2D_DEBUG >= 1
    if (!tolua_isusertable(tolua_S,1,"NetworkManagerFactory",0,&tolua_err)) goto tolua_lerror;
#endif
